...
void proc_init(void)
{
	...
	for (rp = BEG_PROC_ADDR, i = -NR_TASKS; rp < END_PROC_ADDR; ++rp, ++i) {
		...
		rp->p_cpu_virtual_deadline = 0;
		rp->p_cpu_virtual_tick = 0; //
		rp->p_cpu_old_virtual_tick = 0;
 ...
 */
  //int q = rp->p_priority;	 		/* scheduling queue to use */
	...
	rp->p_cpu_virtual_deadline = assign_v_deadline(rp->p_priority);
 ...
 */
  ...
  for (xpp = get_cpu_var_ptr(rp->p_cpu, run_q_head[q]); *xpp;
      ...
      if (*xpp == rp) {				/* found process to remove */
					//if its been dequeued because we blocked, we reuse the old vdeadline
					if (!RTS_ISSET(rp, RTS_NO_QUANTUM)){
						//keep the old vdeadline
						rp->p_cpu_virtual_tick = rp->p_cpu_old_virtual_tick;
					}
 ...
 */
  register struct proc *rp;			/* process to run */
	register struct proc *rp_2_run;
  struct proc **rdy_head;
  int q;				/* iterate over queues */
	int stop_at_this_process; u64_t highest_p_cpu_virtual_deadline=0;
  ...
  rdy_head = get_cpulocal_var(run_q_head);
	...
	for (q=0; q<NR_SCHED_QUEUES;q++){
		...
		//cycle through the processes and find the one with the highest CPU ready Q
		/*
			DIFFERENT SELECTION PROCESS STARTS HERE.
		*/
		highest_p_cpu_virtual_deadline =
			(rp->p_cpu_virtual_deadline - rp->p_cpu_virtual_tick);
		while (rp != NULL){
			//options are
			//pick the process closest (or exceeding) its virtual deadline
			if ((rp->p_cpu_virtual_deadline - rp->p_cpu_virtual_tick) <=
								highest_p_cpu_virtual_deadline){
				highest_p_cpu_virtual_deadline =
					(rp->p_cpu_virtual_deadline - rp->p_cpu_virtual_tick);
				rp_2_run=rp;
				...
				//increase the 'tick'
				rp->p_cpu_virtual_tick++;
			}
			rp=rp->p_nextready;
		}
		rp=rp_2_run;
		/*
			DIFFERENT SELECTION PROCESS FINISHES HERE.
		*/
		//otherwise, we dandy
		assert(proc_is_runnable(rp));
		//the one that runs get its virtual tick bumped down!
		rp->p_cpu_old_virtual_tick=rp->p_cpu_virtual_tick;
		rp->p_cpu_virtual_tick=0;
